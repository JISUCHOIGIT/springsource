...
public class StreamEx7 {
	...
	public static void main(String[] args) {
		...
		//파일명 모아서(map) 확장자가 없는 파일명은 제외하고(filter), 확장자만 모으고(map) 확장자 중복 제거(distinct) 후 확장자 출력(forEach)
		stream.map(File::getName) //"file1.txt","file2.txt","Ex1","Ex2.bak","test.java"
		.filter(f -> f.indexOf(".") > -1 ).map(f -> f.substring(f.lastIndexOf(".") + 1)).distinct().forEach(f -> System.out.println(f));; //"file1.txt","file2.txt","Ex2.bak","test.java"
	}
