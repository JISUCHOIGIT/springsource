...
public class FileCheckTask {
	...
	private String getFolderYesterDay() {
		...
		//추출된 날짜의 포맷 변경 : 문자열 "2022-05-09"
		String str = yesterday.format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
		
		return str.replace("-", File.separator); // "-" 파일 구분하는 구분자 "2022-05-09" => "2022\05\09"으로 return
	}
	...
	public void checkFiles() {
		...
		List<Path> fileListPaths = oldList.stream().map(dto -> Paths.get("d:\\upload", dto.getUploadPath()+"_"+dto.getUuid()+"_"+dto.getFileName())).collect(Collectors.toList());
		
		oldList.stream().filter(dto->dto.isFileType() == true).map(dto->Paths.get("d:\\upload",dto.getUploadPath(),"s_"+dto.getUuid()+"_"+dto.getFileName()))
